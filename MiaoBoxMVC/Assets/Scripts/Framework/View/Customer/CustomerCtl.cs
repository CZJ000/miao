    ...
    void Update() {
        switch (mInnerState) {
            ...
            case EInnerState.GO_AWAY: {
                    ...
                    TransMove();
                    //Move();
                    float disapperaDistance = 1.3f;
                    float remainDistance = (transform.position - mDesPos).magnitude;

                    if(remainDistance < disapperaDistance)
                    {
                        Color fadeColor = Color.white;
                        fadeColor.a = remainDistance / disapperaDistance;
                        SkinnedMeshRenderer[] renderers= gameObject.GetComponentsInChildren<SkinnedMeshRenderer>();
                        foreach(SkinnedMeshRenderer renderer in renderers) {
                            renderer.material.SetColor("_Color", fadeColor);
                        }
                        
                    }
                    if (remainDistance < 0.001f) {

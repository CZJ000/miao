...
public class BuildBlueprintProxy : Proxy
{
    ...
    private NeighborInfoProxy neighborUserInfoProxy;
    ...
    public override void OnRegister()
    {
        ...
        neighborUserInfoProxy = AppFacade.getInstance.RetrieveProxy(NeighborInfoProxy.NAME) as NeighborInfoProxy;
    ...
    public void sendBuildingBlueprintData()
    {
        ...
        SendNotification(LandedEstateMediator.BUILDING_BLUEPRINT_DATA, mBuildBlueprintDict);
        SendNotification(ShopMenuViewMediator.BUILDING_BLUEPRINT_DATA, mBuildBlueprintDict);
    }
    /// <summary>
    /// 发送静态建筑消息
    /// </summary>
    public void sendBuildStatData()
    {
        ...
        SendNotification(LandedEstateMediator.BUILD_STAT_DATA, mBuildDict);
        SendNotification(ShopMenuViewMediator.BUILD_STAT_DATA, mBuildDict);
    }
    /// <summary>
    ...
    public void sendBuildData()
    {
        ...
        SendNotification(LandedEstateMediator.BUILDING_DATA, building);
    }
    /// <summary>
    /// 发送模型初始的信息
    /// </summary>
    public void sendBulidModeldata()
    {
        ...
        neighborUserInfoProxy.sendinitmodeldata();
    }
    /// <summary>
    /// 发送改变模型的信息 
    /// </summary>
    ...
    //根据建筑类型获取不同可雇佣雇员类型
    public List<int> GetRecruittypes(int facility)
    {
        List<int> ret = new List<int>();
        if (mBuildDict.ContainsKey(facility))
        {
            string[] types = mBuildDict[facility].recruittype.Split(',');
            foreach (string type in types)
            {
                int catType = int.Parse(type);
                ret.Add(catType);
            }
        }
        return ret;
    ...
    public void saleBlueprint(System.Object data)
    {
        int selectedItem = (int)data;

        userInfoProxy.consumeBlueprint(selectedItem);
        userInfoProxy.SetGold(userInfoProxy.getGold() + 1);
        sendBuildData();
        //AppFacade.GetInstance().SendNotification(NotiConst.GET_USER_INFO_VALUE);

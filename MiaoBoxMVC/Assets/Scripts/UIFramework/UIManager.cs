...
namespace SUIFW
{
    public class UIManager : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            _TraCanvasTransfrom = GameObject.FindGameObjectWithTag(NotiConst.SYS_TAG_CANVAS).transform;
            _TraCanvasTransfrom.position = new Vector3(0,20,0);
            _TraNormal = Utils.FindTheChildNode(_TraCanvasTransfrom.gameObject, NotiConst.SYS_NORMAL_NODE);
            _TraFixed = Utils.FindTheChildNode(_TraCanvasTransfrom.gameObject, NotiConst.SYS_FIXED_NODE);
            _TraPopUp = Utils.FindTheChildNode(_TraCanvasTransfrom.gameObject, NotiConst.SYS_POPUP_NODE);
            _TraUIScripts = Utils.FindTheChildNode(_TraCanvasTransfrom.gameObject, NotiConst.SYS_SCRIPTMANAGER_NODE);
        ...
        {
           // ResourcesMgr.GetInstance().LoadAsset(NotiConst.SYS_PATH_CANVAS, false);
        }
        ...
        private void PopUIFroms()
        {
            if (_StaCurrentUIForms.Count >= 2)
            {
                ...
                //做隐藏处理
                Debug.Log(topUIForms.gameObject);
                topUIForms.Hiding();
                //出栈后，下一个窗体做“重新显示”处理。
                BaseUIForm nextUIForms = _StaCurrentUIForms.Peek();
                Debug.Log(nextUIForms.gameObject);
        ...
        {
            IConfigManager configMgr = new ConfigManagerByJson(NotiConst.SYS_PATH_UIFORMS_CONFIG_INFO);
            if (configMgr != null)
            {
